...
if __name__ == '__main__':
    ...
    for excel_path in list_file_before:
        ...
        target_qty = pd.melt(source_excel_without_zero,
                             id_vars=['部门', '事务所', '客户匹编码', '项目匹编码', '立账客户', '项目', 'u9料号', '大类', '统计口径', '型号', '核算币种', '汇率', '未税单价', '产品性质'],
                             value_vars=['21年1月数量', '21年2月数量', '21年3月数量', '21年4月数量'],
                             var_name='年月',
                             value_name='数量')
        #删除年月字段最后两个字符
        target_qty['年月'] = target_qty['年月'].str[:-2]
        #金额
        target_mny = pd.melt(source_excel_without_zero,
                             id_vars=['部门', '事务所', '客户匹编码', '项目匹编码', '立账客户', '项目', 'u9料号', '大类',
                                      '统计口径', '型号', '核算币种', '汇率', '未税单价', '产品性质'],
                             value_vars=['21年1月金额', '21年2月金额', '21年3月金额', '21年4月金额'],
                             var_name='年月',
                             value_name='金额'
                             )
        target_mny['年月'] = target_mny['年月'].str[:-2]
        target_with_adjust = pd.concat([target_qty, target_mny['金额']], axis=1)
        target_excel = target_with_adjust[['部门', '事务所', '客户匹编码', '项目匹编码', '立账客户', '项目', 'u9料号', '大类', '统计口径', '型号',
                                           '核算币种', '汇率', '未税单价', '年月', '数量', '金额', '产品性质']]
        # 文件名拆分
